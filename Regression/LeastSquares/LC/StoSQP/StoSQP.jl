...
# X_true, AA: optimal solution, Jacobian
...
function StoSQP(c1,c2,c3,Max_Iter,ttau,nx,nlam,X_true,AA,RR,IdR,EPS=1e-8)
    ...
    t,X_t,Lam_t,NewDir_t = 1,zeros(nx),ones(nlam),zeros(nx+nlam)
    ...
    while t <= Max_Iter+1
        ...
        K_t = hcat(vcat(cum_bar2x_L_t,AA),vcat(AA',zeros(nlam,nlam)))
