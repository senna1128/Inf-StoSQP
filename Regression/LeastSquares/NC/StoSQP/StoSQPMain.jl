	...
	for IdStep=1:LenStep, IdDim=1:LenDim, IdTau=1:LenTau
		...
		for IdSigToe = 1:LenSigToe, IdRep = 1:Rep
			...
			if IdSing == 0
				...
				"Dim",IdDim,"Tau",IdTau,"/Toe",IdSigToe)
				if !isdir(path1)
					mkpath(path1)
				end
		...
		write_matfile(string("../Solution/Step",IdStep,
				"Dim",IdDim,"Tau",IdTau,"/ToeCR.mat");CountConvT,CountRadiusT)
		# Equi Covariance
		CountConvE, CountRadiusE = [[] for i = 1:LenSigEqui], [[] for i = 1:LenSigEqui]
		for IdSigEqui = 1:LenSigEqui, IdRep = 1:Rep
			println("Step","-",IdStep,"-D-",IdDim,"-Tau-",IdTau,"-CovE-",IdSigEqui,"-Rep-",IdRep)
			X1,Lam1,Radius,ErrXLam,ErrK,KKT,Time,IdCov,IdSing=
			StoSQP(c_1,c_2[IdStep],c_3,Max_Iter,tau[IdTau],
			...
			if IdSing == 0
				...
				path1 = string("../Solution/Step",IdStep,
				"Dim",IdDim,"Tau",IdTau,"/Equ",IdSigEqui)
				if !isdir(path1)
					mkpath(path1)
				end
				path = string(path1,"/rep",IdRep,".mat")
		...
		write_matfile(string("../Solution/Step",IdStep,
				"Dim",IdDim,"Tau",IdTau,"/EquCR.mat");CountConvE,CountRadiusE)
	end
