...
function ConstM(SampleSize,nx,nlam,X_true,AA,RR,IdR,EPS=1e-6,Max_Iter=1e3,rho=10,Pen=2)
    ...
    while t <= Max_Iter && KKT >= EPS
        ...
        Quant2 = grad_f'*NewDir_t[1:nx]+rho*(norm(c_val+AA*NewDir_t[1:nx],1)-norm(c_val,1))
        CompFlops += nx*nlam
        while Step_size >= EPS
    ...
    if KKT >= 1e-3
        ...
        ErrK = 0
        K_t = [a_t'*Diagonal(1 ./ ((1 .+ Exp1_t).*(1 .+ Exp2_t)))*a_t/SampleSize+ Pen*Matrix(1.0I,nx,nx) AA'; AA  zeros(nlam, nlam)]
