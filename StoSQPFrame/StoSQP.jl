...
# nlp: model
# Step: step decay ratio
# sigma: estimate variance
# Max_Iter: maximum iterations
# tau: number of inner iterations of the solver
# X_true, Lam_true: optimal solution
# Output:
#  X1, Lam1, Radius, ErrXLam, ErrK, KKT, Time, IdSing

function StoSQP(nlp,Step,sigma2,Max_Iter,tau,X_true,Lam_true,EPS=1e-8)
    nx = nlp.meta.nvar
    nlam = nlp.meta.ncon
    # Compute K^star
    ...
    cum_barggT_t, cum_bar2x_L_t = zeros(nx,nx), Matrix(I,nx,nx)
    ...
    while t <= Max_Iter
        ...
        if abs(R_t[end,end])< EPS || ErrXLam[end]>1e15
            return [],[],[],[],[],[],0,1
        end
        ...
        push!(Radius,[Omega_t[1,1],Omega_t[1,nx+1],Omega_t[nx+1,nx+1]]/(2*(Step<1)+1.5*(Step==1)))
        # Step 1: generate random gradient and Hessian
        g_t,G_tTLam_t,nab_x2L_t = grad(nlp,X_t),jtprod(nlp,X_t,Lam_t),hess(nlp,X_t,Lam_t)
        barg_t = rand(MvNormal(g_t, CovM))
        cum_barggT_t = (1-1/(t+1))*cum_barggT_t + 1/(t+1)*barg_t*barg_t'
        cum_barg_t = (1-1/(t+1))*cum_barg_t + 1/(t+1)*barg_t
        nab_xL_t = g_t + G_tTLam_t
        push!(KKT,norm([nab_xL_t; c_t]))
        bar_nab_L_t = vcat(barg_t+G_tTLam_t, c_t)
        Rand_t = rand(Normal(0,sigma2^(1/2)),nx,nx)
        bar_nab_x2L_t = Hermitian(nab_x2L_t, :L) + (Rand_t+Rand_t')/2
        cum_bar2x_L_t = (1-1/(t+1))*cum_bar2x_L_t+1/(t+1)*bar_nab_x2L_t
        ...
        Step_size = rand(Uniform(2/(t+1)^(Step),2/(t+1)^(Step)+4/(t+1)^(2*Step)))
        X_t = X_t + Step_size*NewDir_t[1:nx]
        Lam_t = Lam_t + Step_size*NewDir_t[nx+1:end]
        t = t + 1
    end
    Time = time() - Time
    return X1, Lam1, Radius, ErrXLam, ErrK, KKT, Time, 0
end
